...
export default () => {
    ...
    function getPointsFor(letter: string) {
      ...
      return lettersAndPoints.reduce((computedScore, pointsTuple) => {
        const [letters, score] = pointsTuple as [string, number]; // declaration with type letters and score
    ...
    // pushToCollection(false, stringCollection); // Error: boolean is not assignable to string
