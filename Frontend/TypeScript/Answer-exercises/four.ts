...
export default () => {
    ...
    function trimmedLength1(text: string | null | undefined) {
      ...
      if (typeof text === "string") {
        text; // text: string not null or undefined
    ...
    function getRandomAnimal() {
      ...
      // Select a random animal from the array and return it
      return animals[Math.floor(Math.random() * animals.length)];
    }
    function interrogateAnimal(
      animal: Animal & { species: string } = getRandomAnimal()
    ) {
      if ("swim" in animal) {
